...
public class MathurakPlanetGenerator extends PlanetGenerator
{
    ...
    ObjectMap<Block, Block> tars = ObjectMap.of(
            FormBlocks.pinksand, FormBlocks.redstone,
            FormBlocks.darkBlueSand, FormBlocks.darkblueStone
    );
    ...
    public Color getColor(Vec3 position){
        ...
        if(block == FormBlocks.darkblueStone) return FormBlocks.darkBlueSand.mapColor;
    ...
    protected void generate(){
                        ...
                        for(int y = -rad; y <= rad; y++){
                            ...
                            if(Structs.inBounds(wx, wy, width, height) && Mathf.within(x, y, rad)){
                                ...
                                if(Mathf.within(x, y, rad - 1) && !other.floor().isLiquid){
                                    ...
                                    other.setFloor((Floor)(floor == FormBlocks.darkBlueSand || floor == FormBlocks.darkblueStone ? FormBlocks.distilledwater : FormBlocks.deepDistilledwater));
                                }
                            }
                        }
                    }
        ...
        pass((x, y) -> {
            ...
            if(value > 0.17f && !Mathf.within(x, y, fspawn.x, fspawn.y, 12 + rrscl)){
                ...
                boolean spore = floor != FormBlocks.darkBlueSand && floor != FormBlocks.darkblueStone;
                ...
                if(!(floor == Blocks.ice || floor == Blocks.iceSnow || floor == Blocks.snow || floor.asFloor().isLiquid)){
                    floor = spore ?
                        (deep ? FormBlocks.deepDistilledwater : FormBlocks.distilledwater) :
                        (deep ? FormBlocks.deepDistilledwater :
                            (floor == FormBlocks.darkBlueSand || floor == FormBlocks.darkblueStone ? FormBlocks.distilledwater : FormBlocks.distilledwater));
                }
            }
        });
        ...
        pass((x, y) -> {
            ...
            if(floor.asFloor().isLiquid && floor.asFloor().shallow){
                ...
                floor = floor == FormBlocks.distilledwater ? FormBlocks.deepDistilledwater : FormBlocks.distilledwater;
            }
        });
        ...
        if(Simplex.noise3d(seed, 2, 0.5, scl, sector.tile.v.x + 1, sector.tile.v.y, sector.tile.v.z)*nmag + poles > 0.5f*addscl){
            ores.add(FormBlocks.orerhodium);
        }
        ...
        pass((x, y) -> {
            ...
            if(floor == FormBlocks.lava){
                if(Math.abs(0.5f - noise(x - 90, y, 4, 0.8, 80)) > 0.035){
                    ...
                    for(Point2 p : Geometry.d4){
                        ...
                        if(other == null || (other.floor() != FormBlocks.lava && other.floor() != FormBlocks.lava)){
                            all = false;
                        }
                    }
                    if(all){
                        floor = FormBlocks.lava;
                    }
                }
                ...
                if(noise > 0.67f && !roomseq.contains(e -> Mathf.within(x, y, e.x, e.y, 14))){
                    if(noise > 0.72f){
                        floor = noise > 0.78f ? FormBlocks.distilledwater : (floor == FormBlocks.darkBlueSand ? FormBlocks.distilledwater : FormBlocks.distilledwater);
                    }else{
                        floor = (floor == FormBlocks.darkBlueSand ? floor : FormBlocks.pinksand);
                    }
                }
            }
            ...
            if(rand.chance(0.0075)){
                ...
                if(any && ((block == FormBlocks.pinksandWall || block == FormBlocks.darkblueStoneWall) || (all && block == Blocks.air && floor == FormBlocks.pinksand && rand.chance(0.03)))){
