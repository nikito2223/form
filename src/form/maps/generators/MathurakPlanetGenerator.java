...
public class MathurakPlanetGenerator extends PlanetGenerator
{
    ...
    protected void generate(){
        ...
        //TODO remove slag and around core.
        ...
        float waveTimerDec = 0.4f;

        state.rules.waveSpacing = Mathf.lerp(60 * 65 * 2, 60f * 60f *1f, Math.max(difficulty - waveTimerDec, 0f));
    ...
    protected void generate(){
                        ...
                        for(int y = -rad; y <= rad; y++){
                            ...
                            if(Structs.inBounds(wx, wy, width, height) && Mathf.within(x, y, rad)){
                                ...
                                if(Mathf.within(x, y, rad - 1) && !other.floor().isLiquid){
                                    Floor floor = other.floor();
                                    //TODO does not respect tainted floors
                                    other.setFloor((Floor)(floor == FormBlocks.redstone || floor == FormBlocks.darkBlueSand ? FormBlocks.distilledwater : FormBlocks.distilledwater));
                                }
                            }
                        }
                    }
        ...
        pass((x, y) -> {
            ...
            if(value > 0.17f && !Mathf.within(x, y, fspawn.x, fspawn.y, 12 + rrscl)){
                ...
                boolean spore = floor != FormBlocks.darkBlueSand && floor != FormBlocks.darkBlueSand;
                //do not place rivers on ice, they're frozen
                //ignore pre-existing liquids
                if(!(floor == FormBlocks.darkBlueSand || floor == FormBlocks.darkBlueSand || floor == FormBlocks.darkBlueSand || floor.asFloor().isLiquid)){
                    floor = spore ?
                        (deep ? FormBlocks.distilledwater : FormBlocks.distilledwater) :
                        (deep ? FormBlocks.distilledwater :
                            (floor == FormBlocks.darkBlueSand || floor == FormBlocks.darkBlueSand ? FormBlocks.distilledwater : FormBlocks.distilledwater));
                }
            }
        });
        ...
        pass((x, y) -> {
            ...
            if(floor.asFloor().isLiquid && floor.asFloor().shallow){
                ...
                floor = floor == FormBlocks.darkBlueSand ? FormBlocks.distilledwater : FormBlocks.distilledwater;
            }
        });
        ...
        if(naval){
            ...
            pass((x, y) -> {
                if(floor.asFloor().isLiquid && !floor.asFloor().isDeep() && !floor.asFloor().shallow){
                    ...
                    floor = floor == FormBlocks.distilledwater ? FormBlocks.distilledwater : FormBlocks.distilledwater;
                }
            });
        }

        Seq<Block> ores = Seq.with(FormBlocks.oreLithium, FormBlocks.oreLithium);
        float poles = Math.abs(sector.tile.v.y);
        float nmag = 0.5f;
        float scl = 1f;
        float addscl = 1.3f;
        ...
        if(rand.chance(0.25)){
            ores.add(FormBlocks.wallOrelithium);
        }
        ...
        pass((x, y) -> {
            ...
            if(rand.chance(0.0075)){
                ...
                if(any && ((block == FormBlocks.darkBlueSand || block == Blocks.iceWall) || (all && block == Blocks.air && floor == FormBlocks.darkBlueSand && rand.chance(0.03)))){
